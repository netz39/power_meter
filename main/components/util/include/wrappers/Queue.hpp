...
public:
    ...
    bool sendToFront(const T &item, TickType_t ticksToWait) const
    {
        return xQueueSendToFront(handle, static_cast<const void *>(&item), ticksToWait) == pdPASS;
    }

    bool sendToFrontFromISR(const T &item, BaseType_t &pxHigherPriorityTaskWoken) const
    {
        return xQueueSendToFrontFromISR(handle, static_cast<const void *>(&item),
                                        &pxHigherPriorityTaskWoken) == pdPASS;
    }
